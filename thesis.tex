...
\begin{figure}
 ...
 To explore the possiblity of using this property of strontium titanate for making thermometers, some STO waver was aquired and cut into a small peice, 3.86 mm long by 3.25 mm wide by 0.5 mm thick. Each side of this wafer was metallized with gold in our Denton evaporator. This sample was then placed in the variable temperature insert probe of our Janis wet cryostat. The capacitance was measured using our Andeen-Hagerling 2700A capactiance bridge while sweeping temperature at a constant rate from 50 K to 2 K. An excitation of 5 V was used to measure the capactiance. The dielectric susceptiblity can be computed from the capactiance and the geometric information about the device using the standard parallel plate capacitor formula from above. The results of this experiment are shown in figure \ref{fig:sto_eps_vs_T}, plotted as $1/\epsilon \approx 1/\epsilon_0 \chi_e$ versus $T^2$. This makes the divergence of the susceptibilty in the quantum critical regime clear as a linear trend. At the lowest temperatures, however, this increase stops, reaching a maximum at 3.29 K. Here, strontium titanate is in the ``quantum paraelectric'' regime, where the gap in the optical phonon disperion is relatively large but the temperature is small compared to the Debye temperature $\theta$~\cite{RowleyThesis}. In this regime the electric susceptiblity stops increasing, as the quantum critical point is no longer having a strong effect. Since the susceptiblity is no longer changing much with temperature, a thermometer made with strontium titanate would not be sensitive in this regime. To illustrate this point, the effective sensitivity of the device is plotted in figure \ref{fig:sto_sens_vs_t} as the quantiy $\frac{1}{C}\frac{dC}{dT}$, where $C$ is the capacitance of the device. This gives a measure of the sensitivity relative to the overall magnetude of the capacitance. The sensitivity peaks at around 24 K, becoming zero at 3.29 K as discussed above.
...
\begin{figure} \caption[Dielectric constant of Potassium Tantalate]{Top: Dielectric constant of Potassium Tantalate
		...
		\ref{fig:sto_eps_vs_T}. The dielectric susceptiblity reaches a
...
As a point of comparison, figure \ref{fig:kto_eps_vs_T} shows a similar experiement with a capacitor made with a different material, Potassium Tantilate (KTaO$_3$). Looking at the phase diagram in figure \ref{fig:qc_rowley}, we can see it is farther away from the quantum critical point, into the quantum paraelectric regime. For this experiement, a 2.26 mm wide by 3.48 mm long by 0.5 mm thick peice of material was metallized into a capacitor. The data is plotted with the same $1/\epsilon$ vs $T^2$ axes as the STO data. By comparing the scale to that in figure \ref{fig:sto_eps_vs_T}, we can see that the the divergence of the dielectric susceptiblity is not nearly as strong, and the dependence not nearly as linear. The quantum paraelectric regime is entered at a much higher temperature, and the susceptiblity reaches it's maximum at a temperature of 4.31 K. Thus, we can see that we must have a material in the quantum critical regime in order to make a good thermometer. In the next chapter, the process of making thermometers out of these devices will be discussed. 
